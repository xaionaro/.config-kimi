...
1. Add the server to `~/.kimi-code/mcp.json` under `mcpServers` (create the file if missing):
   `"openaiDeveloperDocs": { "url": "https://developers.openai.com/mcp" }`
2. Ask the user to restart Kimi (MCP servers load at session start).
