...
Skills are modular, self-contained folders that extend Kimi's capabilities by providing
...
- "Where should I create this skill? If you do not have a preference, I will place it in `$KIMI_CODE_HOME/skills` (or `~/.kimi-code/skills` when `KIMI_CODE_HOME` is unset) so Kimi can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$KIMI_CODE_HOME/skills`; when `KIMI_CODE_HOME` is unset, fall back to `~/.kimi-code/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${KIMI_CODE_HOME:-$HOME/.kimi-code}/skills"
scripts/init_skill.py my-skill --path "${KIMI_CODE_HOME:-$HOME/.kimi-code}/skills" --resources scripts,references
