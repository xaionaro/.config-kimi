...
- **One audience per struct.** A struct joins only fields with the same audience and lifetime. Data consumed independently of the owner (descriptive metadata, shared spec, shared config) forms its own type. The owner's private runtime state (locks, channels, open handles) forms another. Compose by reference or embedding. Do not flatten both into one struct unless one precise domain concept owns the combination. Example: a job's `ID`/`Description`/`Schedule` are standalone descriptive data; the runner's `mu`, `cancel chan` are runtime state — two types, the runner holding the descriptor.
